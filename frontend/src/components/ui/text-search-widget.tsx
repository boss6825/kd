...
export function TextSearchWidget({
    ...
    return (
        <div
            className={`flex flex-col bg-card shadow-[var(--kd-shadow-2)] border border-border rounded-md overflow-hidden min-w-[300px] ${className}`}
        >
            <div className="flex items-center gap-1 p-1">
                <div className="flex-1 relative">
                    <Input
                        ...
                        className="h-8 text-sm w-full pr-[80px] rounded-sm border-border bg-muted/50 focus-visible:ring-0 focus-visible:border-kd-accent placeholder:text-kd-text-3"
                        onKeyDown={(e) => {
                            if (e.key === "Escape") {
                                onClose();
                                onSearchChange("");
