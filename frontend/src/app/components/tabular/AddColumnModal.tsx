...
export function AddColumnModal({ open, existingCount, onClose, onAdd, editingColumn, onSave, onDelete }: Props) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[101] flex items-center justify-center bg-black/50 backdrop-blur-xs">
            <div className="w-full max-w-2xl rounded-2xl bg-card border border-border shadow-[var(--kd-shadow-2)] flex flex-col h-[600px]">
                ...
                <div className="flex items-center justify-between px-6 pt-5 pb-2">
                    <div className="flex items-baseline gap-2 kd-label text-kd-text-3">
                    ...
                    <button
                        onClick={handleClose}
                        className="rounded-lg p-1.5 text-kd-text-3 hover:bg-muted hover:text-foreground transition-colors"
                    >
                    ...
                    <div className="px-6 pt-3 pb-5 space-y-5 overflow-y-auto flex-1">
                            ...
                            >
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    >
                                        <input
                                            ...
                                            className="flex-1 text-2xl font-serif text-foreground placeholder:text-kd-text-3 focus:outline-none bg-transparent"
                                            autoFocus={index === 0}
                                        />
                                        <button
                                            type="button"
                                            ...
                                            className="mt-1.5 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                                        >
                                            <ChevronDown
                                        ...
                                        {presetsOpenIndex === index && (
                                            <div className="absolute left-0 right-0 top-full mt-1 z-50 rounded-[10px] border border-border bg-card shadow-[var(--kd-shadow-2)] overflow-y-auto max-h-64">
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                    ...
                                                    className="w-full px-3 py-2 text-left text-sm text-kd-text-3 hover:bg-muted transition-colors border-b border-border"
                                                >
                                                    No preset
                                                </button>
                                                {PROMPT_PRESETS.map(
                                    ...
                                    {columns.length > 1 && (
                                        <button
                                            ...
                                            className="mt-1.5 rounded-lg p-1.5 text-kd-text-3 transition-colors hover:bg-muted hover:text-foreground"
                                        >
                                            <X className="h-4 w-4" />
                                        </button>
                                    )}
                                ...
                                <div className="mt-4">
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button className="mt-1 flex items-center justify-between rounded-[10px] border border-border bg-card px-2 py-1.5 text-sm text-foreground hover:border-kd-text-3 focus:outline-none">
                                                <span className="flex items-center gap-2">
                                                    {(() => {
                                                        ...
                                                        return (
                                                            <Icon className="h-3.5 w-3.5 text-kd-text-3" />
                                                        );
                                                    })()}
                                                    {formatLabel(column.format)}
                                                </span>
                                                <ChevronDown className="h-3.5 w-3.5 text-kd-text-3" strokeWidth={1.5} />
                                            </button>
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent
                                            align="start"
                                            ...
                                            >
                                                {FORMAT_OPTIONS.map((o) => (
                                                    <DropdownMenuRadioItem
                                                        ...
                                                        value={o.value}
                                                    >
                                                        <o.icon className="h-3.5 w-3.5 text-kd-text-3" />
                                                        {o.label}
                                                    </DropdownMenuRadioItem>
                                                ))}
                                            </DropdownMenuRadioGroup>
                                ...
                                {column.format === "tag" && (
                                    <div className="mt-3">
                                        ...
                                        <div className="mt-1 flex flex-wrap gap-1.5 rounded-[10px] border border-border px-2 py-1.5 focus-within:border-kd-accent">
                                            ...
                                            <input
                                                ...
                                                placeholder="Add tag…"
                                                className="min-w-[80px] flex-1 bg-transparent text-sm text-foreground placeholder:text-kd-text-3 focus:outline-none"
                                            />
                                        </div>
                                        <p className="mt-1 text-xs text-kd-text-3">
                                ...
                                <div className="mt-4 flex items-center justify-between">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground disabled:text-kd-text-3"
                                    >
                                        {generatingIndices.includes(index) ? (
                                ...
                                <textarea
                                    ...
                                    placeholder="Write the analysis prompt — describe what KD should extract from each document for this column…"
                                    className="mt-2 w-full rounded-[10px] border border-border px-3 py-2 text-sm text-foreground placeholder:text-kd-text-3 focus:border-kd-accent focus:outline-none bg-transparent resize-none leading-relaxed"
                                />
                            </div>
                        ))}
                        ...
                        {!isEditing && (
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-[10px] border border-dashed border-border px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground hover:border-kd-text-3"
                            >
                                <Plus className="h-4 w-4" strokeWidth={1.5} />
                                Add another column
                            </button>
                    ...
                    <div className="flex items-center justify-between border-t border-border px-6 py-4">
                        <div>
                            {isEditing && onDelete && (
                                <button
                                    type="button"
                                    onClick={onDelete}
                                    className="rounded-[10px] px-4 py-2 text-sm text-kd-danger hover:bg-kd-danger/10 transition-colors"
                                >
                                    Delete
                                </button>
                            )}
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                type="button"
                                onClick={handleClose}
                                className="rounded-[10px] px-4 py-2 text-sm text-muted-foreground hover:bg-muted transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="rounded-[10px] bg-primary px-5 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-40 transition-opacity"
                            >
