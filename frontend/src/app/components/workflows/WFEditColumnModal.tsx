            ...
            <div className="w-full max-w-2xl rounded-2xl border border-border bg-card shadow-[var(--kd-shadow-2)] flex flex-col h-[600px]">
                ...
                <div className="flex items-center justify-between px-6 pt-5 pb-2">
                    <div className="kd-label flex items-center gap-2 text-kd-text-3">
                        <span>Workflows</span>
                        <span>/</span>
                        <span className="text-muted-foreground">Edit column</span>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
                    ...
                    <div className="px-6 pt-3 pb-5 overflow-y-auto flex-1">
                        ...
                        <div className="flex items-start gap-2">
                            <div className="relative flex flex-1 items-start" ref={presetsRef}>
                                <input
                                    ...
                                    className="flex-1 bg-transparent font-serif text-2xl text-foreground placeholder:text-kd-text-3 focus:outline-none"
                                    autoFocus
                                />
                                <button
                                    type="button"
                                ...
                                >
                                    <ChevronDown className={`h-4 w-4 transition-transform ${presetsOpen ? "rotate-180" : ""}`} strokeWidth={1.5} />
                                </button>
                                {presetsOpen && (
                                    <div className="absolute left-0 right-0 top-full mt-1 z-50 max-h-64 overflow-y-auto rounded-xl border border-border bg-card shadow-[var(--kd-shadow-2)]">
                                        <button
                                            type="button"
                                            onClick={() => { update({ name: "", prompt: "", format: "text", tags: [], tagInput: "" }); setPresetsOpen(false); }}
                                            className="w-full border-b border-border px-3 py-2 text-left text-sm text-kd-text-3 transition-colors hover:bg-muted"
                                        >
                                            No Preset
                                        </button>
                        ...
                        <div className="mt-4">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className="mt-1 flex items-center justify-between rounded-[10px] border border-border bg-card px-2.5 py-1.5 text-sm text-foreground transition-colors hover:border-kd-text-3 focus:outline-none">
                                        <span className="flex items-center gap-2">
                                            <FormatIcon className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.5} />
                                    ...
                                    >
                                        {FORMAT_OPTIONS.map((o) => (
                                            <DropdownMenuRadioItem key={o.value} value={o.value}>
                                                <o.icon className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.5} />
                                                {o.label}
                                            </DropdownMenuRadioItem>
                                        ))}
                                    </DropdownMenuRadioGroup>
                        ...
                        {draft.format === "tag" && (
                            <div className="mt-3">
                                ...
                                <div className="mt-1 flex flex-wrap gap-1.5 rounded-[10px] border border-border px-2.5 py-1.5 focus-within:border-kd-accent">
                                ...
                                <p className="mt-1 text-xs text-kd-text-3">Press Enter or comma to add a tag.</p>
                            </div>
                        )}
                        ...
                        <div className="mt-4 flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 text-sm text-kd-accent transition-colors hover:text-kd-accent-strong disabled:text-kd-text-3"
                            >
                                {generating ? (
                                    <span className="block h-4 w-4 animate-spin rounded-full border-2 border-border border-t-kd-accent" />
                                ) : (
                        ...
                        <textarea
                            ...
                            className="mt-2 w-full resize-none rounded-[10px] border border-border bg-transparent px-3.5 py-2.5 text-sm leading-relaxed text-foreground placeholder:text-kd-text-3 focus:border-kd-accent focus:outline-none"
                        />
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-border px-6 py-4">
                        <button
                            type="button"
                            onClick={onDelete}
                            className="rounded-[10px] px-4 py-2 text-sm text-destructive transition-colors hover:bg-destructive/10"
                        >
                            Delete
                        </button>
