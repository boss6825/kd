...
import { listWorkflows } from "@/app/lib/kdApi";
import { BUILT_IN_WORKFLOWS } from "../workflows/builtinWorkflows";

interface Props {
    open: boolean;
    onClose: () => void;
    onSelect: (workflow: KdWorkflow) => void;
    projectName?: string;
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-foreground/10 backdrop-blur-xs">
            ...
            >
                ...
                <div className="flex flex-row flex-1 min-h-0 overflow-hidden">
                        ...
                        {loading ? (
                            ...
                            filteredWorkflows.map((wf) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 text-xs text-left transition-colors border-b border-border/40 ${
                                        selected?.id === wf.id
                                            ? "bg-muted/50"
                                            : "hover:bg-muted/50"
                                    }`}
                    ...
                    {selected && (
                        <div className={`flex-1 border-l border-border/60 flex flex-col overflow-hidden px-3 pb-3 transition-opacity duration-200 ${rightVisible ? "opacity-100" : "opacity-0"}`}>
                            <div className="flex items-center justify-between py-3 shrink-0">
                                ...
                                <button
                                    ...
                                    className="rounded-lg p-1 text-muted-foreground/70 hover:bg-muted hover:text-muted-foreground transition-colors"
                                >
                                    <ChevronLeft className="h-3.5 w-3.5" />
                                </button>
                            </div>
                            <div className="flex-1 overflow-y-auto px-4 py-3 text-sm border border-border rounded-md text-muted-foreground leading-relaxed font-serif bg-muted/50">
                                <ReactMarkdown
