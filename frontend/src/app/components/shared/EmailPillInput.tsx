...
export function EmailPillInput({
    ...
    return (
        <div>
            <div
                className={`flex flex-wrap gap-1.5 rounded-[10px] border bg-card px-3 py-2 min-h-[40px] transition-colors ${
                    error
                        ? "border-kd-danger/50 focus-within:border-kd-danger"
                        : "border-border focus-within:border-kd-accent"
                }`}
            >
                {emails.map((email) => (
                    <span
                ...
                <input
                    ...
                    className="flex-1 min-w-[160px] bg-transparent text-sm text-foreground placeholder:text-kd-text-3 outline-none"
                    autoFocus={autoFocus}
                />
            </div>
