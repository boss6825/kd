// Shared TypeScript types for KD AI legal assistant

export interface KdFolder {
  id: string;
  project_id: string;
  user_id: string;
  name: string;
...
export interface KdEditAnnotation {
  type?: "edit_data";
  kind?: "edit";
  edit_id: string;
  document_id: string;
...
export type AssistantEvent =
  ...
  | {
        ...
        version_number?: number | null;
        download_url: string;
        annotations: KdEditAnnotation[];
        error?: string;
        isStreaming?: boolean;
    }
  | { type: "content"; text: string; isStreaming?: boolean };

export interface KdMessage {
  role: "user" | "assistant";
  content: string;
 ...
 */
export interface KdCitationAnnotation {
  type: "citation_data";
  ref: number;
  doc_id: string;
  document_id: string;
...
export function formatCitationPage(a: KdCitationAnnotation): string {
  if (typeof a.page === "string") return `Page ${a.page}`;
  return `Page ${a.page}`;
}

/** Produce a reader-friendly version of the quote (replaces [[PAGE_BREAK]] with "..."). */
export function displayCitationQuote(a: KdCitationAnnotation): string {
...
export interface KdChatDetailOut {
  chat: KdChat;
  messages: KdMessage[];
}
