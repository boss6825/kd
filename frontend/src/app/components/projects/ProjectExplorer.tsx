...
import type { KdDocument, KdFolder } from "@/app/components/shared/types";
...
interface Props {
    ...
    onDocClick: (doc: KdDocument) => void;
...
export function ProjectExplorer({
    ...
    async function handleDropOnTarget(targetFolderId: string | null, e: React.DragEvent) {
        ...
        const movingFolderId = e.dataTransfer.getData("application/kd-folder");
    ...
    function isInternalDrag(e: React.DragEvent): boolean {
        return (
            Array.from(e.dataTransfer.types).includes("application/kd-doc") ||
            Array.from(e.dataTransfer.types).includes("application/kd-folder")
        );
    }
        ...
        return (
            <>
                ...
                {creatingIn === parentId && (
                    ...
                    >
                        <ChevronRight className="h-3 w-3 text-kd-text-3 shrink-0" />
